...
var stylesheet =
    '.drag-active .drop-container{position:relative}.drag-active .drop-container *{pointer-events:none}.drag-active .drop-container:before{position:absolute;top:0;right:0;bottom:0;left:0;z-index:9999;content:""}';

mod.factory('dragContext', [
    function() {
        var context = {};
...
mod.run([
    ...
    '$rootElement',
    '$timeout',
    function($document, $rootElement, $timeout) {
        $document[0].addEventListener('dragend', onDragEnd, true);
        $document[0].addEventListener('drop', onDrop, true);
...
mod.directive('dragContainer', [
    '$rootElement',
    '$parse',
    '$timeout',
    'dragContext',
    function($rootElement, $parse, $timeout, dragContext) {
        return {
            restrict: 'A',
            link: function($scope, $element, $attrs) {
                var onDragStart = $attrs.onDragStart
                    ? $parse($attrs.onDragStart)
                    : null;
                ...
                function handleDragStart(e) {
                    ...
                    if (targetEvent.dataTransfer) {
                        if (
                            (!targetEvent.dataTransfer.items ||
                                !targetEvent.dataTransfer.items.length) &&
                            (!targetEvent.dataTransfer.files ||
                                !targetEvent.dataTransfer.files.length)
                        ) {
...
mod.directive('dropContainer', [
    '$document',
    '$parse',
    '$window',
    'dragContext',
    function($document, $parse, $window, dragContext) {
        return {
            restrict: 'A',
            ...
            link: function($scope, $element, $attrs, dropContainer) {
                var acceptsFn = $attrs.dropAccepts
                    ? $parse($attrs.dropAccepts)
                    : function($scope, locals) {
                          return typeof locals.$dragData !== 'undefined';
                      };
                var onDragEnter = $attrs.onDragEnter
                    ? $parse($attrs.onDragEnter)
                    : null;
                var onDragOver = $attrs.onDragOver
                    ? $parse($attrs.onDragOver)
                    : null;
                var onDragLeave = $attrs.onDragLeave
                    ? $parse($attrs.onDragLeave)
                    : null;
                ...
                function handleDragEnter(e) {
                    if (
                        ...
                        dragContext.lastTarget !== $element
                    ) {
                        dragContext.lastTarget.$attrs.$removeClass('drag-over');
                    }
                ...
                function handleDragOver(e) {
                    ...
                    if (acceptsFn($scope, locals)) {
                        ...
                        Angular.forEach(dropContainer.targets, function(
                            dropTarget,
                            anchor
                        ) {
                            var anchorX = width / 2;
                ...
                function handleDrop(e) {
                    ...
                    if (dropContainer.lastTarget) {
                        $attrs.$removeClass(
                            'drop-container-' + dropContainer.lastTarget.anchor
                        );
                    }
        ...
        function offset(elem) {
            ...
            return {
                width: Math.round(
                    Angular.isNumber(elemBCR.width)
                        ? elemBCR.width
                        : elem.offsetWidth
                ),
                height: Math.round(
...
mod.controller('DropContainerController', [
    function() {
        ...
        dropContainer.detach = function(dropTarget) {
            ...
            if (!dropContainer.targets[anchor] === dropTarget) {
                throw new Error(
                    'The indicated drop target is not attached at ' +
                        'the anchor `' +
                        anchor +
                        '`.'
                );
    ...
    function($parse) {
        return {
            ...
            link: function($scope, $element, $attrs, ctls) {
                ...
                $attrs.$addClass(
                    'drop-target drop-target-' + dropTarget.anchor
                );

                dropContainer.attach(dropTarget);

                var onDragEnter = dropTarget.$attrs.onDragEnter
                    ? $parse(dropTarget.$attrs.onDragEnter)
                    : Angular.noop;
                var onDragLeave = dropTarget.$attrs.onDragLeave
                    ? $parse(dropTarget.$attrs.onDragLeave)
                    : Angular.noop;
                var onDragOver = dropTarget.$attrs.onDragOver
                    ? $parse(dropTarget.$attrs.onDragOver)
                    : Angular.noop;
                var onDrop = dropTarget.$attrs.onDrop
                    ? $parse(dropTarget.$attrs.onDrop)
                    : Angular.noop;
...
function findDragActiveTarget(jqLite) {
    var body = jqLite.find('body');

    return body.length ? body : jqLite;
}
